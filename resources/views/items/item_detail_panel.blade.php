...
<div class="row">
    <div class="col-4 offset-1">
        <img class="card-img-top" src="/images/item-images/{{ $item->image_file_name }}">
    </div>
    <div class="col-6">
        <table class="table table-bordered">
            <tr>
                ...
                <td>
                    @if (!empty($item->seller->avatar_file_name))
                        <img src="/images/avatars/{{ $item->seller->avatar_file_name }}" class="rounded-circle"
                            style="object-fit: cover; width: 35px; height: 35px;">
                    @else
                        <img src="/images/avatars/avater.png" class="rounded-circle"
                            style="object-fit: cover; width: 35px; height: 35px;">
                    @endif
                    {{ $item->seller->name }}
                </td>
            ...
            <tr>
                <th>カテゴリー</th>
                <td>{{ $item->secondaryCategory->primaryCategory->name }} / {{ $item->secondaryCategory->name }}
                </td>
            </tr>
            <tr>
                <th>商品の状態</th>
